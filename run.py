...
# 训练模型
def train_model(model, train_dataloader, epochs=1):
...
# 创建词汇表和标签映射
word_to_ix = {word: i for i, word in enumerate(set(word for sentence in texts for word in sentence))}
tag_to_ix = {tag: i for i, tag in enumerate(set(tag for tags in ner_tags for tag in tags))}
